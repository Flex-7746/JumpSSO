...
module.exports = async function login(opt) {
  ...
  try {
    ...
    const target = await ctx.$db.Config.findOne({
      where: { status: 1, key: "dingding_config" },
    ...
    const [err1, access_token] = await getToken({ ctx, corpId: config?.corpId, clientId: config?.clientId, clientSecret: config?.clientSecret });
    if (err1) {
      return [err1];
    }
    ...
    const result = {
      ...
      nickname: user[attribute.nickname || "remark"],
      picture: user[attribute.picture || "avatar"],
    };

    if (config.saveInfo) {
      const target = await ctx.$db.User.findOne({ where: { status: 1, channel: 3, openid: result.openid }, attributes: ["id"] });
      ...
      if (target) {
        ...
        await ctx.$db.User.create({ channel: 3, password: "", role: 1, ...result });
      }
    }
